...
import {
  ...
  Loader2, ShieldCheck, Briefcase, FileText, ChevronRight,
  UserCog, Building2,
} from "lucide-react";
...
// Build URL helpers
function buildFilter(base: string, stage: string, status: string): string {
  return `${base}?stage=${encodeURIComponent(stage)}&status=${encodeURIComponent(status)}`;
}
function buildRole(base: string, role: string): string {
  return `${base}?role=${encodeURIComponent(role)}`;
}
function buildUploader(base: string, uid: string): string {
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const u3 = onSnapshot(collection(db, "job_requisitions"), snap => {
      setJobReqs(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });
    return () => { u1(); u2(); u3(); };
  }, []);

  // ── Filtered candidates for charts ──
  const filtered = useMemo(() => {
    ...
    return candidates.filter(c => {
      if (filterRole !== "all") {
        // Filter by uploader's role — match candidates whose createdBy user has this role
  ...
  // ── Per-agency breakdown: each agency user + how many candidates they uploaded ──
  const agencyBreakdown = useMemo(() => {
    const agencyUsers = users.filter(u => (u.role || "").toLowerCase() === "agency");
    return agencyUsers.map(u => ({
      id:    u.id,
      name:  u.displayName || u.name || u.email || "Unknown Agency",
      email: u.email || "",
      count: candidates.filter(c => c.createdBy === u.id).length,
    })).sort((a, b) => b.count - a.count);
  }, [users, candidates]);

  // ── Per-HR breakdown: each HR user + how many candidates they uploaded ──
  const hrBreakdown = useMemo(() => {
    const hrUsers = users.filter(u => (u.role || "").toLowerCase() === "hr");
    return hrUsers.map(u => ({
      id:    u.id,
      name:  u.displayName || u.name || u.email || "Unknown HR",
      email: u.email || "",
      count: candidates.filter(c => c.createdBy === u.id).length,
    })).sort((a, b) => b.count - a.count);
  }, [users, candidates]);
  ...
  const uniqueRoles = useMemo(() =>
    Array.from(new Set(candidates.map(c => c.candidateDesignation).filter(Boolean))) as string[],
  [candidates]);
  ...
  const stats = useMemo(() => ({
    ...
    // keep below using filtered (for UI breakdown)
  ...
  const pipelineTotal = useMemo(() =>
    stats.l1Selected + stats.l2Selected + stats.hrSelected + stats.completed, [stats]);
    ...
    filtered.forEach(c => {
      [
        ...
        if (c[st] === "Scheduled" && ds && ds >= todayStr && ds <= maxStr) {
          list.push({ id:`${c.id}-${key}`, candidateName: c.candidateName || "Unknown", jobRole: c.candidateDesignation || "N/A", roundLabel: label, date: ds, timeSlot: c[sf] || "", isToday: ds === todayStr, candidateId: c.id });
        }
      });
    });
  ...
  return (
    <div className="space-y-6 pb-10 w-full overflow-x-hidden">
      ...
      {/* ══════════════════════════════
          LIVE INDICATOR (no duplicate title — title comes from layout)
      ══════════════════════════════ */}
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-end">
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Uploaded by Role</label>
            ...
            <div className="space-y-1.5">
              ...
              <Select onValueChange={setFilterStatus} value={filterStatus}>
                ...
                <SelectContent>
                  ...
                  {["l1","l2","hr"].includes(filterStage) && <><SelectItem value="Scheduled">Scheduled</SelectItem><SelectItem value="Selected">Selected</SelectItem><SelectItem value="Rejected">Rejected</SelectItem><SelectItem value="Pending">Pending</SelectItem></>}
      ...
      {/* ══════════════════════════════
          ROW 1: CANDIDATE OVERVIEW — 4 clickable stat cards
      ══════════════════════════════ */}
      <div>
      ...
      {/* ══════════════════════════════
          ROW 2: USER MANAGEMENT SUMMARY
          Shows HR/Panel/Agency counts — click goes to /users?role=...
      ══════════════════════════════ */}
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          {/* HR Users */}
          <Link href={buildRole(ROUTES.userManagement, "hr")} className="block group">
            <Card className="border hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800">
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-blue-500">Manage HR team</p>
                    </div>
                  </div>
                ...
                {hrBreakdown.length > 0 && (
                  <div className="space-y-1 pt-2 border-t border-blue-200 dark:border-blue-800">
                    {hrBreakdown.slice(0, 3).map(hr => (
                      <button key={hr.id}
                        onClick={(e) => { e.preventDefault(); router.push(buildUploader(ROUTES.candidateHistory, hr.id)); }}
                        className="w-full flex items-center justify-between text-[11px] py-0.5 hover:text-blue-700 transition-colors group/item">
                        <span className="truncate text-muted-foreground group-hover/item:text-blue-600 max-w-[140px]">{hr.name}</span>
                        <span className="font-bold text-blue-600 shrink-0">{hr.count} candidates</span>
                      </button>
                    ))}
                    {hrBreakdown.length > 3 && (
                      <p className="text-[10px] text-muted-foreground text-center pt-1">+{hrBreakdown.length - 3} more</p>
                    )}
                  </div>
                )}
              </CardContent>
          ...
          {/* Panel Users */}
          <Link href={buildRole(ROUTES.userManagement, "panel")} className="block group">
            <Card className="border hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer bg-violet-50 dark:bg-violet-950/20 border-violet-200 dark:border-violet-800">
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-violet-500">Handle interviews</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="text-2xl font-black text-violet-700 dark:text-violet-400">{userCounts.panel}</span>
                    <ArrowUpRight className="h-4 w-4 text-violet-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                </div>
          ...
          {/* Agency Users */}
          <Link href={buildRole(ROUTES.userManagement, "agency")} className="block group">
            <Card className="border hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800">
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                ...
                {/* Per-Agency upload counts — each clickable */}
                {agencyBreakdown.length > 0 && (
                  <div className="space-y-1 pt-2 border-t border-amber-200 dark:border-amber-800">
                    {agencyBreakdown.slice(0, 4).map(agency => (
                      <button key={agency.id}
                        onClick={(e) => { e.preventDefault(); router.push(buildUploader(ROUTES.candidateHistory, agency.id)); }}
                        className="w-full flex items-center justify-between text-[11px] py-0.5 hover:text-amber-700 transition-colors group/item">
                        <span className="truncate text-muted-foreground group-hover/item:text-amber-600 max-w-[140px]">{agency.name}</span>
                        <span className="font-bold text-amber-600 shrink-0">{agency.count} candidates</span>
                      </button>
                    ))}
                    {agencyBreakdown.length > 4 && (
                      <p className="text-[10px] text-muted-foreground text-center pt-1">+{agencyBreakdown.length - 4} more</p>
                    )}
                  </div>
                )}
              </CardContent>
      ...
      {/* ══════════════════════════════
          ROW 4: STAGE BREAKDOWN — all 5 stages, every pill clickable
      ══════════════════════════════ */}
      <div>
          ...
          <CardContent className="pt-5 px-8 pb-5 relative">
            {upcoming.length > 0 ? (
              <Carousel opts={{ align:"start" }} className="w-full">
                ...
                {upcoming.length > 4 && <><CarouselPrevious className="-left-5" /><CarouselNext className="-right-5" /></>}
              </Carousel>
            ) : (
