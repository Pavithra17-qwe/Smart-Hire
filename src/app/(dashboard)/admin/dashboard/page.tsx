...
export default function AdminDashboard() {
  ...
  const filtered = useMemo(() => {
    return candidates.filter(c => {
      ...
      const agencyMatch = filterAgency === "all" || c.agencyId === filterAgency;
      const statusMatch = filterStatus === "all" || c.finalStatus === filterStatus;
      return projectMatch && agencyMatch && statusMatch;
    });
  }, [candidates, filterProject, filterAgency, filterStatus]);
  ...
  const hasActiveFilters = filterProject !== "all" || filterAgency !== "all" || filterStatus !== "all";
    ...
    <div className="space-y-8 pb-10 w-full overflow-x-hidden dashboard-container">
      <div className="flex flex-col md:flex-row items-end gap-4 bg-card p-5 rounded-xl shadow-sm border">
        <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase text-muted-foreground tracking-wider ml-1">Agency</label>
            <Select onValueChange={setFilterAgency} value={filterAgency}>
              <SelectTrigger className="rounded-lg h-11"><SelectValue placeholder="All Agencies" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All Agencies</SelectItem>
                {agencies.map(a => <SelectItem key={a.id} value={a.agencyId}>{a.name}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
